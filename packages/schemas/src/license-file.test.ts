 ...
 * so `license` is npm's documented pointer form instead of an expression.
 * packages/schemas/LICENSE.md is a copy of the repository root LICENSE.md (npm can only pack files
 * inside the package directory); this test is what keeps the copy from drifting.
 */
const packageDir = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const repoRoot = resolve(packageDir, '..', '..');
const read = (path: string) => readFileSync(path, 'utf8');

describe('@adgateio/schemas licence', () => {
