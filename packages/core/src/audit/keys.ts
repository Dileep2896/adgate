...
/** How many hex characters of the digest a fingerprint keeps. 64 bits is plenty to spot a swap. */
export const KEY_FINGERPRINT_HEX_CHARS = 16;

/**
 * A short, stable name for a public key: the first 16 hex characters of the SHA-256 of its SPKI
 * DER bytes. The key is decoded first, so the same key always fingerprints the same however its
 * PEM was wrapped, escaped or newline-terminated - which is the point. A deployment logs it at
 * boot next to key_id, and a key that silently changed under an unchanged key_id (a restored
 * backup, a second instance generated by a scripted deploy) shows up as a different fingerprint
 * in the deploy log instead of as unverifiable records weeks later.
 *
 * Reproduce it from a PEM anywhere:
 *   openssl pkey -pubin -in key.pem -outform DER | shasum -a 256 | cut -c1-16
 *
 * Public keys only: it is not a secret, and passing a private PEM throws (loadPublicKey checks
 * the header) so a fingerprint can never be computed from key material that must not be logged.
 */
export const keyFingerprint = (publicPem: string): string => {
  const der = loadPublicKey(publicPem).export({ type: 'spki', format: 'der' });
  return sha256Hex(der).slice(0, KEY_FINGERPRINT_HEX_CHARS);
};
